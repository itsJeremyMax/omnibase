...
export async function handleExecuteSql(
  ...
  // Matches with optional schema prefix (e.g., pg_catalog.pg_shadow, dbo.syslogins).
  // For entries that already include a dot (mysql.user, sys.sql_logins), matches exactly.
  ...
  if (
    ...
    // Extract table names from the query (check common patterns)
